...
package com.biermacht.brews.DragDropList;
...
public class DragSortListView extends ListView {
  ...
  /**
   * As opposed to {@link android.widget.ListView#getAdapter()}, which returns a heavily wrapped
   * ListAdapter (DragSortListView wraps the input ListAdapter {\emph and} ListView wraps the
   * wrapped one).
   *
   ...
   *
   * @param which
   *         Position to remove (NOTE: headers/footers ignored! this is a position in your input
   *         ListAdapter).
   * @param velocityX
   */
   ...
   * com.biermacht.brews.DragDropList.DragSortListView.DropListener#drop(int, int)}.
   *
   * @param from
   ...
   *         <code>dragFlags |= ~{@link #DRAG_NEG_X}</code> to allow dragging the floating View in
   *         all directions except off the screen to the left.
   * @param deltaX
   ...
   *         touch-y minus float View top).
   * @return True if the drag was started, false otherwise. This <code>startDrag</code> will fail if
   * we are not currently in a touch event, <code>floatView</code> is null, or there is a drag in
   * progress.
   */
  ...
  public interface FloatViewManager {
     ...
     * {@link android.view.ViewGroup.LayoutParams} on this View; otherwise it will set some for you
     * (with a width of FILL_PARENT and a height of WRAP_CONTENT).
     *
     * @param position
     *         Position of item to drag (NOTE: <code>position</code> excludes header Views; thus, if
     *         you want to call {@link android.widget.ListView#getChildAt(int)}, you will need to
     *         add {@link android.widget.ListView#getHeaderViewsCount()} to the index).
     ...
     *         The floating View.
     * @param location
     *         The location (top-left; relative to DSLV top-left) at which the float View would like
     *         to appear, given the current touch location and the offset provided in {@link
     *         com.biermacht.brews.DragDropList.DragSortListView#startDrag}.
     * @param touch
   ...
   * sense to. Make sure {@link android.widget.BaseAdapter#notifyDataSetChanged()} or something like
   * it is called in your implementation. Furthermore, if you have a choiceMode other than none and
   * the ListAdapter does not return true for {@link android.widget.ListAdapter#hasStableIds()}, you
   * will need to call {@link #moveCheckState(int, int)} to move the check boxes along with the list
   * items.
   *
   * @param l
   */
   ...
   * android.widget.BaseAdapter#notifyDataSetChanged()} or something like it. When an item removal
   * occurs, DragSortListView relies on a redraw of all the items to recover invisible views and
   * such. Strictly speaking, if you remove something, your dataset has changed...
   *
   * @param l
   */
   ...
   * android.widget.BaseAdapter#notifyDataSetChanged()} or something like it in your
   * implementation.
   *
   * @author heycosmo
   */
  ...
  /**
   * Make sure to call {@link android.widget.BaseAdapter#notifyDataSetChanged()} or something like
   ...
   * constant in time. Create your own by implementing {@link com.biermacht.brews.DragDropList.DragSortListView.DragScrollProfile}.
   *
   * @param ssp
   */
   ...
   * <p/>
   * A word of warning about a "feature" in Android that you may run into when dealing with movable
   * list items: for an adapter that <em>does</em> have stable IDs, ListView will attempt to locate
   * each item based on its ID and move the check state from the item's old position to the new
   * position — which is all fine and good (and removes the need for calling this function), except
   * for the half-baked approach. Apparently to save time in the naive algorithm used, ListView will
   * only search for an ID in the close neighborhood of the old position. If the user moves an item
   * too far (specifically, more than 20 rows away), ListView will give up and just force the item
   * to be unchecked. So if there is a reasonable chance that the user will move items more than 20
   * rows away from the original position, you may wish to use an adapter with unstable IDs and call
   * this method manually instead.
   *
   * @param from
   * @param to
   */
   ...
   * com.biermacht.brews.DragDropList.DragSortListView#setDragScrollProfile(com.biermacht.brews.DragDropList.DragSortListView.DragScrollProfile)}
  ...
  public interface DragScrollProfile {
     ...
     *
     * @param w
     *         Normalized position in scroll region (i.e. w \in [0,1]). Small w typically means slow
     *         scrolling.
     * @param t
