...
/**
 * A subclass of {@link android.widget.CursorAdapter} that provides reordering of the elements in
 * the Cursor based on completed drag-sort operations. The reordering is a simple mapping of list
 * positions into Cursor positions (the Cursor is unchanged). To persist changes made by drag-sorts,
 * one can retrieve the mapping with the {@link #getCursorPositions()} method, which returns the
 * reordered list of Cursor positions.
 * <p/>
 * An instance of this class is passed to {@link DragSortListView#setAdapter(ListAdapter)} and,
...
public abstract class DragSortCursorAdapter extends CursorAdapter implements DragSortListView.DragSortListener {
   ...
   * is unchanged. Retrieve the current mapping with {@link getCursorPositions()}.
   *
